...
uv run python3 -m datasets.build_corpus --dataset hotpotqa --input-path .temp/hotpot_wikipedia.tar.bz2

echo "\n\nBuilding 2WikiMultiHopQA corpus\n"
mkdir -p data/corpus/2wikimultihopqa
uv run python3 -m datasets.build_corpus --dataset 2wikimultihopqa

echo "\n\nBuilding MuSiQue corpus\n"
mkdir -p data/corpus/musique
uv run python3 -m datasets.build_corpus --dataset musique
