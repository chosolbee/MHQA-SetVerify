...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Multiple JSON/JSONL files to merge for training (and auto-split for validation if not provided)."
    )
    parser.add_argument(
        "--validation_split_percentage", type=int, default=10,
        help="Percentage for validation split when using --train_files and no --validation_file."  # NEW
    )
...
def main():
        ...
        def _read_jsonlines(fp):
            ...
            with open(fp, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if "id" not in ex and "question_id" in ex:
                        ex["id"] = ex["question_id"]

                    if "dataset_name" not in ex:
                        ex["dataset_name"] = Path(fp).stem
                    items.append(ex)
            return items

        for fp in args.train_files:
            all_data.extend(_read_jsonlines(fp))

        if len(all_data) == 0:
            raise ValueError("No examples loaded from --train_files")

        rng.shuffle(all_data)
        val_size = max(1, int(len(all_data) * (args.validation_split_percentage / 100.0)))
    ...
    data_collator = DataCollatorForSeq2Seq(
        ...
        pad_to_multiple_of=8 if getattr(accelerator, "mixed_precision", None) in ("fp16", "bf16") else None,
    )
    ...
    if args.with_tracking:
        ...
        project_name = os.environ.get("WANDB_PROJECT", "classifier") 
                        ...
                        accelerator.log(
                            {
                                ...
                                "train/lr": lr_scheduler_train.get_last_lr()[0],
                            },
    ...
    if args.do_eval:
        ...
        gold_answers = eval_examples[args.answer_column]
